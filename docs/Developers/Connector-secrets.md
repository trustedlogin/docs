...
For Cloudflare specifically, along with a full explanation of the trust model, see [Running behind a reverse proxy or CDN](./Connector-running-behind-a-proxy.md) — it includes a copy-paste snippet covering all Cloudflare edge CIDRs and guidance for nginx / Apache / HAProxy / cloud load balancers.
